...
import { Ruler, CheckCircle, Info, HelpCircle } from "lucide-react";
...
export default function Chapter5() {
  ...
  const groups = {
    positive: {
      ...
      cards: LENORMAND_CARDS.filter((c) => c.type === "positive")
    },
    "positive-neutral": {
    ...
    <div className="space-y-6">
      ...
      <div className="text-center max-w-3xl mx-auto space-y-3">
        <h2 className="text-2xl font-bold text-primary font-esoteric tracking-wider flex items-center justify-center gap-2">
          <Ruler className="w-6 h-6" /> POSITIVE TO NEGATIVE SPECTRUM
        </h2>
      ...
      <Card className="bg-card/20 border-primary/20 p-4 backdrop-blur-md">
        <div className="relative h-16 w-full bg-gradient-to-r from-emerald-500/20 via-amber-500/20 to-rose-500/20 rounded-lg border border-primary/10 flex items-center justify-between px-4 sm:px-8 select-none">
          {/* Ruler markings */}
      ...
      <div className="flex justify-center">
        <Tabs value={activeGroup} onValueChange={setActiveGroup} className="w-full max-w-3xl">
          <TabsList className="grid grid-cols-3 sm:grid-cols-6 bg-card/40 border border-primary/20 p-1 h-auto gap-1">
            ...
            <TabsTrigger value="positive" className="text-xs font-esoteric py-2 text-emerald-400">
              POSITIVE
            </TabsTrigger>
            <TabsTrigger value="positive-neutral" className="text-xs font-esoteric py-2 text-teal-400">
              POS-NEU
            </TabsTrigger>
            <TabsTrigger value="neutral" className="text-xs font-esoteric py-2 text-amber-400">
              NEUTRAL
            </TabsTrigger>
            <TabsTrigger value="neutral-negative" className="text-xs font-esoteric py-2 text-orange-400">
              NEU-NEG
            </TabsTrigger>
            <TabsTrigger value="negative" className="text-xs font-esoteric py-2 text-rose-400">
              NEGATIVE
            </TabsTrigger>
          </TabsList>
        </Tabs>
      ...
      {activeGroup === "all" ? (
        <div className="space-y-6">
          {Object.entries(groups).map(([key, group]) => (
            <Card key={key} className="bg-card/20 border-primary/10 backdrop-blur-sm p-4">
          ...
          {/* Rule Note */}
          <blockquote className="border-l-2 border-primary/50 pl-4 py-2 text-xs text-primary/80 italic font-serif-kr bg-primary/5 rounded-r max-w-3xl mx-auto space-y-1">
            <div className="font-bold flex items-center gap-1"><Info className="w-3.5 h-3.5" /> 꼭 기억해야 할 예외 및 보충 규칙</div>
            <p className="text-[11px] leading-relaxed pl-4.5">
              • <strong>신사(Lord)와 숙녀(Lady)</strong> 카드는 리딩의 대상(주인공)이 되므로 이 수치 스펙트럼에서 제외됩니다.<br />
        ...
        <Card className="bg-card/20 border-primary/10 backdrop-blur-sm p-6">
