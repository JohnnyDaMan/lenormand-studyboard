...
import { HelpCircle, CheckCircle2, RefreshCw, AlertCircle, Compass } from "lucide-react";
...
interface StudyQuizQuestion {
  ...
  correctIdx: number;
  explanation: string;
  cards: string[]; // Cards to display for reference (const.ts name 기준)
}
  ...
  const questions: StudyQuizQuestion[] = [
    {
      ...
      questionEn: "In the combination [ Man – Letter – Ship – Bouquet – Fish ], when 'Man' mirrors 'Fish', what additional detail is revealed?",
      options: [
        "이 남성은 편지를 보내는 집배원이다.",
      ...
      correctIdx: 1,
      explanation: "신사(Man)와 물고기(Fish)는 가로 5장 배열의 양 끝에 위치하여 서로 거울처럼 마주 봅니다. 물고기는 재정 및 풍요를 상징하므로, 신사가 물고기를 미러링하는 것은 이 남성이 부유하거나 사업가임을 구체화해 줍니다.",
      cards: ["Man", "Letter", "Ship", "Bouquet", "Fish"]
    },
    {
      id: 2,
      questionKr: "다음 체인(Chain) 리딩에서 새-숙녀-채찍 카드가 '반지'와 '하트' 사이에 놓여 있습니다. 올바른 해석은 무엇인가요?\n[ 반지 – 새 – 숙녀 – 채찍 – 하트 ]",
      questionEn: "In the chain [ Ring – Birds – Lady – Whip – Heart ], what is the correct interpretation?",
      options: [
      ...
      cards: ["Ring", "Birds", "Lady", "Whip", "Heart"]
    }
  ];
    ...
    <div className="space-y-6">
      ...
      <div className="text-center max-w-3xl mx-auto space-y-3">
        <h2 className="text-2xl font-bold text-primary font-esoteric tracking-wider flex items-center justify-center gap-2">
          <Compass className="w-6 h-6 text-primary animate-spin-slow" /> EXERCISE #3 : ADVANCED METHODS QUIZ
        </h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
            ...
            <CardContent className="space-y-4 font-serif-kr">
              ...
              <div className="space-y-2">
                {currentQuestion.options.map((option, idx) => {
                  ...
                  let optionStyle = "bg-background border-primary/10 hover:bg-primary/5 text-foreground/80";
                  if (isSelected) {
                    optionStyle = "bg-primary/10 border-primary text-primary font-semibold";
                  }
                  if (showResult) {
                    if (isCorrect) {
                      optionStyle = "bg-emerald-50 border-emerald-300 text-emerald-800 font-semibold shadow-sm";
                    } else if (isSelected) {
                      optionStyle = "bg-rose-50 border-rose-300 text-rose-800";
                    } else {
                      optionStyle = "bg-background border-primary/5 text-foreground/30";
                    }
                  }
                  ...
                  return (
                    <button
                      ...
                      disabled={showResult}
                      className={`w-full text-left p-3 rounded-lg border text-xs transition-all duration-200 leading-relaxed flex items-start gap-2 cursor-pointer shadow-sm ${optionStyle}`}
                    >
              ...
              {showResult && (
                <div className={`p-3.5 rounded-lg border text-xs space-y-2 animate-fade-in shadow-sm ${
        ...
        <div className="lg:col-span-6">
          <Card className="bg-card/50 border border-primary/10 p-6 flex flex-col justify-center items-center h-full shadow-inner">
            ...
            <div className="flex justify-center items-center gap-2 flex-wrap">
              {currentQuestion.cards.map((cname, idx) => {
                const card = LENORMAND_CARDS.find((c) => c.name.toLowerCase() === cname.toLowerCase());
                const isMirroredOrTheme = 
                  currentQuestion.id === 1 && (idx === 0 || idx === 4) || // Man mirrors Fish
                ...
                return card ? (
                  <div key={card.id} className="relative">
                    ...
                    />
                    {isMirroredOrTheme && (
                      <div className="absolute -top-2 left-1/2 -translate-x-1/2 bg-primary text-primary-foreground text-[8px] font-bold px-1.5 py-0.5 rounded-full shadow-md font-esoteric whitespace-nowrap z-20">
